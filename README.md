...
| | |
|---|---|
| **自動保存** | 打つたびにブラウザの中に控えます（打ち終えて 0.7 秒、打ち続けていても 3 秒ごと）。閉じても次に開けば続きから |
| **作品** | シナリオをいくつでもブラウザの中に置いておけます。一覧から開く・新しく作る・複製・過去の版・削除（ゴミ箱） |
| **保存**（`Ctrl` + `S`） | `.json` ファイルとして手元に残します。これが原稿の本体です。Chrome / Edge では以後、自動保存のたびにそのファイルにも書きます |
| **開く** | `.json` を、新しい作品として読み込みます（いまの作品は上書きしません） |
...
| しくみ | 守るもの |
|---|---|
| 閉じる瞬間の控え | タブを閉じる・再読み込みする瞬間に保存しきれなかった分を控え、次に開いたときに戻します |
| 3 秒ごとの保存 | 打ち続けていても 3 秒ごとに書くので、ブラウザがいきなり落ちても、消えるのは最後の数秒分までです |
| 1 作品 1 タブ | 同じ作品は 1 つのタブでしか開けません（「別のタブで編集中」）。2 つのタブで上書きし合うことがありません |
| 過去の版 | 作品ごとに、開いたとき・書いているあいだ 10 分ごと・「全部消して最初から」の直前の姿を 10 個まで残します |
| ゴミ箱 | 削除した作品は 30 日のあいだ戻せます |
| 失敗の帯 | 保存に失敗すると、画面の上に赤い帯が出て、直るまで消えません |
| ファイルへの自動保存 | Chrome / Edge では、はじめの **保存** で選んだファイルにも自動で書きます。ブラウザのデータが消えても、そのファイルから戻せます |

> 作品はブラウザの IndexedDB に置きます。ブラウザのデータを消すと作品も消えるので、
> 大切な原稿は **保存** でファイルにも残してください（Chrome / Edge なら一度選べば以後は自動です）。
>
> v3.1 までの自動保存は、はじめて v3.2 以降を開いたときに作品として引き継ぎます。
